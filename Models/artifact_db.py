"""
Simplified SQLite database - 4 entities only
"""
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS persons (
        ...
        birth_year INTEGER,
        death_year INTEGER,
        canonical_id TEXT REFERENCES persons(id),
        external_ids TEXT, -- JSON
        created_at TEXT DEFAULT CURRENT_TIMESTAMP
    );
    CREATE INDEX IF NOT EXISTS idx_persons_name ON persons(name);
    CREATE INDEX IF NOT EXISTS idx_persons_canonical ON persons(canonical_id);
    CREATE INDEX IF NOT EXISTS idx_persons_birth_death ON persons(birth_year, death_year);
    ...
    CREATE TABLE IF NOT EXISTS locations (
        ...
        street_name TEXT,
        cross_streets TEXT, -- JSON array
        neighborhood TEXT,
        borough TEXT,
        year_start INTEGER,
        year_end INTEGER,
        geometry TEXT, -- WKT
        canonical_id TEXT REFERENCES locations(id),
        external_ids TEXT, -- JSON
        created_at TEXT DEFAULT CURRENT_TIMESTAMP
    );
    CREATE INDEX IF NOT EXISTS idx_locations_name ON locations(name);
    CREATE INDEX IF NOT EXISTS idx_locations_canonical ON locations(canonical_id);
    CREATE INDEX IF NOT EXISTS idx_locations_address ON locations(normalized_address);
    CREATE INDEX IF NOT EXISTS idx_locations_street ON locations(street_name);
    CREATE INDEX IF NOT EXISTS idx_locations_neighborhood ON locations(neighborhood, borough);
    ...
    CREATE INDEX IF NOT EXISTS idx_context_chunks_book ON context_chunks(book_id);
...
def stats():
    """Show row counts"""
    conn = get_connection()
    tables = ['books', 'persons', 'locations', 'context_chunks', 'events', 'milestones', 'entity_matches']
    stats = {}
    for table in tables:
